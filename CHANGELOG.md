...
## [1.0.6] - 2026-09-21
### Added
- Added listening on volume change using fsnotify library.

## [1.0.5] - 2026-09-20
### Changed
- Moved copy/export logic out of the `App` struct into standalone functions in `copy.go`.
- Moved shared `MediaFile` type to `types.go`.

## [1.0.4] - 2026-09-20
### Added
- Versioning skill (`version`) that automates SemVer bumps and changelog entries.
- `AGENTS.md` with repository guidance (project layout, verification, test data, skills).

## [1.0.3] - 2026-03-16
### Added
- Support for `DVR_EXPORT_PATH` environment variable as the default export destination.
### Changed
- Export only video format files (e.g. .mp4, .mov, .mkv) and subtitle files (.srt), excluding image files.
